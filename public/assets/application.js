...
(function() {
  var addOptions, addRow, createElement, findResult, foodRow, newFoodEaten, newFoodPurchased, purchasedFoodRow, searchConsumed;

  this.eatenFoodCount = 0;

  this.storedFoodCount = 0;

  this.purchasedFoodCount = 0;
  ...
  searchConsumed = function() {
    ...
    return $.ajax({
      ...
      success: function(results) {
        var display, result, _i, _len, _ref, _results;
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          _results.push((function(result) {
            ...
            return display.append(newRow);
          })(result));
        }
        return _results;
      },
      error: function(xhr, status, errorThrown) {
        return window.alert("Status: " + status + "; Error: " + errorThrown);
      }
    });
  };
  ...
  newFoodEaten = function(event) {
    var beforeRow, newRow, result;
    window.eatenFoodCount++;
    result = findResult(event.target.id);
    newRow = foodRow("consumed_food", this.eatenFoodCount, result);
    beforeRow = $('#consumed_foods');
    return addRow(newRow, beforeRow);
  };

  newFoodPurchased = function() {
    var beforeRow, newRow;
    this.purchasedFoodCount++;
    newRow = purchasedFoodRow();
    beforeRow = document.getElementById("add_food_purchase");
    return addRow(newRow, beforeRow);
  };

  addRow = function(row, before) {
    return row.insertBefore(before);
  };

  addOptions = function(select, options) {
    var element, option, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = options.length; _i < _len; _i++) {
      element = options[_i];
      option = createElement("option");
      option.innerHTML = element;
      _results.push(select.appendChild(option));
    }
    return _results;
  };

  foodRow = function(name, count, result) {
    var foodName, frequencyLabel, frequencyOptions, frequencySelect, nameLabel, newRow, number, option, servingsLabel, servingsSelect, time, _i, _j, _len;
    ...
    for (number = _i = 1; _i <= 10; number = _i += 1) {
    ...
    for (_j = 0, _len = frequencyOptions.length; _j < _len; _j++) {
      time = frequencyOptions[_j];
  ...
  purchasedFoodRow = function() {
    var amountInput, foodAmountLabel, foodFrequencyLabel, foodNameLabel, foodTimeUnitLabel, frequencyInput, nameInput, newRow, option, timeSelect, timeUnits, unit, unitSelect, units, _i, _j, _len, _len1;
    ...
    nameInput.setAttribute("name", "purchased_foods[food" + this.purchasedFoodCount + "[name]]");
    ...
    amountInput.setAttribute("name", "purchased_foods[food" + this.purchasedFoodCount + "[amount]]");
    unitSelect = createElement("select");
    unitSelect.setAttribute("name", "purchased_foods[food" + this.purchasedFoodCount + "[unit]]");
    ...
    frequencyInput.setAttribute("name", "purchased_foods[food" + this.purchasedFoodCount + "[frequency_amount]]");
    ...
    timeSelect.setAttribute("name", "purchased_foods[food" + this.purchasedFoodCount + "[time_unit]]");
    timeUnits = ["day", "week", "two weeks", "month", "year"];
    for (_j = 0, _len1 = timeUnits.length; _j < _len1; _j++) {
      unit = timeUnits[_j];
      option = createElement("option");
      option.innerHTML = unit;
  ...
  $(document).ready(function() {
    return $('#consumed_food_search').bind('change', searchConsumed);
  });
